...
namespace RxjhServer
{
	public class 武功类 : IDisposable
	{
		...
		{
			if (World.TBL_KONGFU.TryGetValue(id, out var value))
			{
		...
		{
			武功类 value;
			return (!World.TBL_KONGFU.TryGetValue(wgid, out value) || value == null || Playe.武功新[value.FLD_武功类型, value.FLD_INDEX] == null || Playe.武功新[value.FLD_武功类型, value.FLD_INDEX].FLD_PID != wgid) ? null : Playe.武功新[value.FLD_武功类型, value.FLD_INDEX];
		}

		public static bool GetsfeWg(Players Playe, int wgid)
		{
			武功类 value;
			return World.TBL_KONGFU.TryGetValue(wgid, out value) && value != null && Playe.武功新[value.FLD_武功类型, value.FLD_INDEX].FLD_PID == wgid;
		}
