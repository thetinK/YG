...
namespace RxjhServer
{
	public class 追加状态类 : IDisposable
	{
		...
		public void 时间结束事件1()
		{
			...
			else if (!Play.退出中 && Play.Client.Running)
			{
				try
				{
				...
				{
					MainForm.WriteLine(1, "追加武功状态类 时间结束事件 出错：" + num + "[" + FLD_PID + "]" + ex);
				}
				finally
				{
					Dispose();
			...
			else if (!Play.退出中 && Play.Client.Running)
			{
				try
				{
					...
					case 1000000674:
						...
						break;
					case 1000000675:
						Play.FLD_人物_追加_攻击 -= 500;
						Play.FLD_人物_追加_防御 -= 500;
						Play.人物追加最大_HP -= 3000;
						Play.更新武功和状态();
						Play.更新HP_MP_SP();
						break;
					...
					case 700350:
						Play.追加状态列表.Remove(FLD_PID);
						Play.delFLD_追加百分比_防御(Play.狂意护体加成);
						Play.更新武功和状态();
						Play.狂意护体加成 = 0.0;
						Dispose();
						return;
					...
					case 1008001159:
					{
						...
						players.药品_追加_首饰_强化 -= FLD_PID - 1008001155;
						Play.追加状态列表.Remove(FLD_PID);
						Play.计算人物装备数据();
						Play.更新武功和状态();
						break;
					...
					case 1008001163:
						num = 12;
						Play.delFLD_装备_追加_武器_强化(Play.追加项链属性);
						Play.delFLD_装备_追加_防具_强化(Play.追加项链属性);
						Play.追加状态列表.Remove(FLD_PID);
						Play.计算人物装备数据();
						Play.更新武功和状态();
						break;
					...
					{
						Players play2 = Play;
						Players players = play2;
						players.药品追加对怪攻击 -= 100;
						play2 = Play;
						players = play2;
						players.药品追加对怪防御 -= 100;
						Play.更新武功和状态();
						break;
					}
					case 1008000096:
					...
					case 1008000248:
						Play.FLD_人物_追加_经验百分比 -= 1.0;
						Play.FLD_人物_追加_获得游戏币百分比 -= 1.0;
						Play.FLD_人物_追加_物品掉落概率百分比 -= 1.0;
						Play.FLD_人物_追加_历练百分比 -= 1.0;
						if (Play.FLD_人物_追加_经验百分比 < 0.0)
						{
							Play.FLD_人物_追加_经验百分比 = 0.0;
					...
					case 1008002064:
						...
						Play.FLD_人物_追加_攻击 -= 200;
						Play.FLD_人物_追加_防御 -= 200;
						Play.更新武功和状态();
						Play.更新HP_MP_SP();
						break;
					case 1008002386:
				...
				{
					MainForm.WriteLine(1, "追加状态类 时间结束事件 出错：" + num + "[" + FLD_PID + "]" + ex);
				}
				finally
				{
					Dispose();
