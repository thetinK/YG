	...
	public class 离线打架系统
	{
		...
		private static void 打架地图读取(Players player)
		{
			if (player.人物坐标_地图 != 101)
			{
				...
				int num2 = player.获取随机数2(1100, 1250);
				player.自动挂机坐标X = num;
				player.自动挂机坐标Y = num2;
				player.自动挂机地图 = 101;
		...
		public static void 离线攻击包(Players player)
		{
			...
			Buffer.BlockCopy(BitConverter.GetBytes(player.人物全服ID), 0, array, 4, 2);
			Buffer.BlockCopy(BitConverter.GetBytes(player.离线打架攻击人物), 0, array, 10, 2);
			Buffer.BlockCopy(BitConverter.GetBytes(player.离线打架武功ID), 0, array, 14, 4);
		...
		public static int 获取自动攻击NPC目标(Players player, float float_0, float float_1)
		{
			...
			{
				if (value.人物_HP > 0 && !value.Player死亡 && !value.Client.挂机 && player.查找范围玩家(50, value) && player.人物全服ID != value.人物全服ID)
				{
					result = value.人物全服ID;
				}
			}
