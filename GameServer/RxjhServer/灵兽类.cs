...
namespace RxjhServer
{
	public class 灵兽类 : IDisposable
	{
		...
		public int 灵兽基本攻击 => (int)((double)(FLD_攻击 + FLD_装备_追加_攻击) * (1.0 + FLD_追加百分比_攻击));
		...
		public int 灵兽基本回避 => (int)((double)(FLD_回避 + FLD_装备_追加_回避) * (1.0 + FLD_追加百分比_回避));
		...
		public 灵兽类(long id, NetState clien, DataTable table2, Players Playe)
		{
			...
			for (int k = 0; k < 32; k++)
			{
				...
				try
				{
					...
					Buffer.BlockCopy(array3, k * 4, array4, 0, 4);
					int num = BitConverter.ToInt32(array4, 0);
					if (num == 0)
					{
						continue;
					}
					武功类 武功类2 = new 武功类(num);
					if (武功类2.FLD_JOB != 0)
					{
						switch (武功类2.FLD_JOB)
						{
